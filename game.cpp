...
string Game ::getFirstPlayer(){
    return this ->pl1.getName();
}

string Game ::getSecondPlayer(){
    return this ->pl2.getName();
}
...
void Game :: handleRoundResult(Card& c1, Card& c2, Player& pl1, Player& pl2, vector<Card>& adding) {
    if (c1.getNumber() == 1 && c2.getNumber() != 2) {
        // pl1 wins the round
        pl1.setNumWins(pl1.getNumWins() + 1);
...
void Game :: playTurn(){
    ...
    // Check if war can be AVOIDED
    compareCardsAndSetWinner(c1, c2, pl1, pl2, winner);
    ...
    if (c1.getNumber() == c2.getNumber()) {
        ...
        while (c1.getNumber() == c2.getNumber()){
            ...
            handleRoundResult(c1, c2, pl1, pl2, adding);
            }
